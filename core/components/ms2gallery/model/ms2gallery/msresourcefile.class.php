...
class msResourceFile extends xPDOSimpleObject {
    ...
    public function saveWithWaterMark($tmp, $src) {
        $params = $this->xpdo->fromJSON('{"w":960,"h":540,"q":95,"zc":"1","bg":"000000","fltr":"wmi|/assets/templates/chiptuner/images/watermark.png|BR|80"}');
        // Подключаем обёртку MODX, которая выставит системные настройки в класс phpThumb
        $phpThumb = $this->xpdo->getService('modphpthumb','modPhpThumb', MODX_CORE_PATH . 'model/phpthumb/', array());
        // Указываем исходник
        ...
        if ($phpThumb->GenerateThumbnail()) {
            ...
            if (!$phpThumb->renderToFile($src)) {
                $this->xpdo->log(modX::LOG_LEVEL_ERROR, 'Could not save rendered image to'.$src);
            }
        }
        else {
            // Если возникла ошибка - пишем лог работы в журнал MODX
            $this->xpdo->log(modX::LOG_LEVEL_ERROR, print_r($phpThumb->debugmessages, 1));
        }
    }
}